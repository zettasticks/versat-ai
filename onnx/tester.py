...
def ExtractModelOperatorsTestableInfo(cModel):
    ...
    for op in cModel.operations:
        ...
        if opName in opToAttributesTested:
            opToAttributesTested[opName] = JoinAttributes(
                opToAttributesTested[opName], attributes
            )
        else:
    ...
    for opName, attributesTested in opToAttributesTesedDict.items():
        ...
        for attrName, tested in attributesTested.items():
            ...
            if spec.attrType == OnnxAttributeType.INTEGER:
                print(
                    f"  {attrName} is tested in {differentTests} different ways: {testedContent}"
                )
                ...
                if differentTests == allAllowedAmount:
                    print(
                        f"  {attrName} is fully tested (possible values: {spec.allowedValues})"
                    )
                else:
                    print(
                        f"  {attrName} is tested in {differentTests} different ways: {testedContent} (missing {allAllowedAmount - differentTests} tests)"
                    )
...
report = ExtractModelOperatorsTestableInfo(
    GenerateModelFromOnnxModel(onnx.load(sys.argv[1]))
)
