...
FSX = b"\x09"  # Send file size request
...
def GetFileAndSize(resourceDir, name):
    file = None
    file_size = None

    try:
        with chdir(resourceDir):
            file = open(name, "rb")
        ...
        try:
            ...
            print(f"Could not find file named: {name}")
            print("Looked at paths:")
            print(f"  {resourceDir}")
            print(f"  {os.getcwd()}")
            sys.exit(0)
...
def cnsl_sendfile(resourceDir):
    ...
    # open file to send
    f, file_size = GetFileAndSize(resourceDir, name)
    print(PROGNAME, end="")
...
def cnsl_sendfilesize(resourceDir):
    file_size = 0
    name = b""
