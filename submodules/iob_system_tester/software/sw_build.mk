...
# Tester has a memwrapper peripheral, but drivers for it are provided by iob_regfileif
# So rename iob_memwrapper to iob_regfileif
PERIPHERALS := $(PERIPHERALS:iob_memwrapper=iob_regfileif)
...
UTARGETS+=build_iob_system_tester_software tb
...
IOB_SYSTEM_TESTER_FW_SRC+=$(addprefix src/,$(addsuffix _csrs.c,$(PERIPHERALS)))
...
IOB_SYSTEM_TESTER_BOOT_SRC+=src/iob_uart_csrs.c
...
iob_system_tester_preboot:
	make $@.elf INCLUDES="$(IOB_SYSTEM_TESTER_INCLUDES)" LFLAGS="$(IOB_SYSTEM_TESTER_LFLAGS) -Wl,-Map,$@.map" SRC="$(IOB_SYSTEM_TESTER_PREBOOT_SRC)" TEMPLATE_LDS="$(TEMPLATE_LDS)" NO_HW_DRIVER=1
