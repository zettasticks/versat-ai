...
void *Software_Conv2(void *inputX, void *inputW,void *inputB, void *output, int index,
                    ConvInfo *info) {
  ...
  float *outViewFloat = (float *)output;
  ...
  for (int batch = 0; batch < batches; batch++) {
    ...
    float *outView = outViewFloat + (batch * outputSize);
    ...
    for (; Address_IsValid(outGen); Address_Advance(outGen)) {
      ...
      int kernelIndex = outC * extra.kernelW * extra.kernelH;
