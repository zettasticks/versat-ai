...
void *Software_ConvWithBias(void *inputX, void *inputW, void *inputB,
  ...
  AddressGen outGenInst = StartAddress(outputDims, outputDims, info->dims);
...
void *Software_Transpose(void *data, void *output, int index,
                         TransposeInfo *info) {
  int64_t *inputDims = VERSAT_TransposeInfo_inputDims(info);
  int64_t *perm = VERSAT_TransposeInfo_perm(info);

  int64_t outDims[MAX_DIMS] = {};

  for (int i = 0; i < info->numberInputDims; i++) {
    int index = perm[i];
    outDims[i] = inputDims[index];
  }

  AddressGen in = StartAddress(inputDims, inputDims, info->numberInputDims);
...
void *Software_Add(void *inputA, void *inputB, void *output, int index,
  ...
  int64_t *firstInputDim = VERSAT_AddInfo_firstInputDims(info);
  int64_t *secondInputDim = VERSAT_AddInfo_secondInputDims(info);
  int64_t *broadCastedShape = VERSAT_AddInfo_broadCastedShape(info);
  ...
  AddressGen inA = StartAddress(broadCastedShape, firstInputDim, info->maxDims);
  AddressGen inB =
      StartAddress(broadCastedShape, secondInputDim, info->maxDims);
  AddressGen outGen =
      StartAddress(broadCastedShape, broadCastedShape, info->maxDims);
