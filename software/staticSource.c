...
AddressGen StartAddress(int64_t* iterationDims,int64_t* properDims,int numberDims){
  AddressGen gen = {};
...
void* Reshape(void* data,void* shape,void* output,int index,ReshapeInfo* info){
  ...
  AddressGen in = StartAddress(info->inputDims,info->inputDims,info->numberInputDims);
  ...
  // TODO: Looking at the indexes produced, it appears that the Reshape operation does not need to copy data around, altought I still need to look further into this. 
  while(IsValid(&out)){
